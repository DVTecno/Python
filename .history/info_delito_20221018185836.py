...
with open("Sacramento_crime_Jan2006.txt") as delitos_file:
    ...
    for delito in delitos_file:
        ...
        delito_count = delito_count.get(delito_nom)
        if delito_count:
            delito_count[delito_nom] = delito_count+1
        else:
            delito_count[delito_nom]=1
delito_count_ordered=sorted(((value,key) for (key,value) in delito_count.item()),reverse=True)
