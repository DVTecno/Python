...
with open("Sacramento_crime_Jan2006.txt") as delitos_file:
    ...
    for event in delitos_file:
        delito_event = event.split(sep=',')
        delito_nom = delito_event[5].replace('\n', '')
...
delito_count_ordered=sorted(((value,key) for (key,value) in delitos_count.items()),reverse=True)
